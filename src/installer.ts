...
export const findCompatibleVersion = (pkg: string): string | null => {
  try {
    ...
    const peerDependencyOutput = execSync(`npm show ${pkg} peerDependencies --json`)
      .toString()
      .trim();
    ...
    console.log(
      `Searching for a version of ${pkg} that works with ${peerDepName}@${installedPeerVersion}`,
    );

    const availableVersions = JSON.parse(
      execSync(`npm show ${pkg} versions --json`).toString().trim(),
    );
    ...
    for (let version of availableVersions.reverse()) {
      ...
      try {
        const peerDepsForVersionOutput = execSync(
          `npm show ${pkg}@${version} peerDependencies --json`,
        )
          .toString()
          .trim();
        if (peerDepsForVersionOutput && peerDepsForVersionOutput !== '{}') {
          ...
          if (
            parsedPeerDeps[peerDepName] &&
            parsedPeerDeps[peerDepName].includes(installedPeerVersion)
          ) {
            return version;
          }
        }
...
export const installMissingDependencies = (dependencies: string[], isTs = false): void => {
  ...
  const missingPackages = validPackages.filter(
    (pkg) => !Object.keys(installedPackages).includes(pkg),
  );
  ...
  if (missingPackages.length === 0 && outdatedPackages.length === 0) {
      ...
      missingPackages.forEach((pkg) => {
        try {
          ...
          if (error.message.includes('could not resolve dependency')) {
            console.warn(
              `Peer dependency conflict detected for ${pkg}. Finding a compatible version...`,
            );
            ...
            if (compatibleVersion) {
              ...
              try {
                ...
                console.log(
                  `Successfully installed compatible version: ${pkg}@${compatibleVersion}`,
                );
              } catch (error) {
    ...
    if (outdatedPackages.length > 0) {
      ...
      try {
        execSync(`npm install ${outdatedPackages.map((pkg) => `${pkg}@latest`).join(' ')}`, {
          stdio: 'inherit',
        });
